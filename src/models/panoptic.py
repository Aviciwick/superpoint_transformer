...
class PanopticSegmentationModule(SemanticSegmentationModule):
    ...
    def forward(
        ...
        若启用 H-SPT，会在特征提取和粗语义预测后：
        1. 使用 AUS 筛选困难超点
        2. 使用 CAFM 增强特征
        3. 使用 RRH 输出点级预测
        4. 使用增强后的特征重新计算语义预测和 edge affinity
        """
        # Extract features
        x = self.net(nag)

        # Recover level-1 features
        sp_features = x[0] if self.multi_stage_loss else x

        # Compute coarse semantic predictions
        semantic_pred = [head(x_) for head, x_ in zip(self.head, x)] \
            if self.multi_stage_loss else self.head(x)

        # 粗分类 logits（用于 H-SPT 不确定性评估）
        coarse_logits = semantic_pred[0] if self.multi_stage_loss else semantic_pred
        ...
        if self.hspt_enabled and self.hspt is not None:
            ...
            try:
                from src.hspt import build_packed_points
                ...
                if sp_centroids is not None:
                    ...
                    if hspt_output.fused_features is not None:
                        ...
                        if self.multi_stage_loss:
                            semantic_pred = [self.head[0](sp_features)] + \
                                            [head(x_) for head, x_ in zip(self.head[1:], x[1:])]
                        else:
                            semantic_pred = self.head(sp_features)
        ...
        # Recover level-1 features（如被 H-SPT 更新则使用增强版本）
        x_l1 = sp_features
        ...
        x_edge = x_l1[nag[1].obj_edge_index]
        x_edge = torch.cat(
